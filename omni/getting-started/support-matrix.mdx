...
If you are running a self-hosted version of Omni licensed under the BSL, please regularly [update](../cluster-management/upgrading-cluster#upgrading-omni-clusters) to the latest release - we suggest at least monthly. Bug fixes will not be backported to older versions of Omni, so support that involves a bug fix will require an update.
