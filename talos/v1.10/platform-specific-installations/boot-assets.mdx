...
* adding [system extensions](../build-and-extend-talos/custom-images-and-development/system-extensions.mdx)
* updating [kernel command line arguments](../reference/kernel.mdx)
* using custom `META` contents, e.g. for [metal network configuration](../networking/metal-network-configuration.mdx)
* generating [SecureBoot](./bare-metal-platforms/secureboot.mdx) images signed with a custom key
...
[Image Factory](../learn-more/image-factory.mdx) is a service that generates Talos boot assets on-demand.
...
* <a href={`https://factory.talos.dev/image/b8e8fbbe1b520989e6c52c8dc8303070cb42095997e76e812fa8892393e1d176/${release}/metal-amd64.iso`}>
    using ISO image (download it and burn to a CD/DVD or USB stick)
  </a> 

* <a href={`https://factory.talos.dev/pxe/b8e8fbbe1b520989e6c52c8dc8303070cb42095997e76e812fa8892393e1d176/${release}/metal-amd64`}>
...
Now we can [download the metal arm64 image](https://factory.talos.dev/image/0db665edfda21c70194e7ca660955425d16cec2aa58ff031e2abf72b7c328585/${release}/metal-arm64.raw.xz) (download it and burn to a boot media)
...
Now the `aws-amd64.raw.xz` file contains the customized Talos AWS disk image which can be uploaded as an AMI to the [AWS](./cloud-platforms/aws.mdx).
...
The `imager` container image can be checked by [verifying its signature](../security/verifying-images.mdx).
...
* `iso` builds a Talos ISO image (see [ISO](./bare-metal-platforms/iso.mdx))
* `secureboot-iso` builds a Talos ISO image with SecureBoot (see [SecureBoot](./bare-metal-platforms/secureboot.mdx))
...
* `secureboot-installer` builds an installer container image with SecureBoot (see [SecureBoot](./bare-metal-platforms/secureboot.mdx))
...
* `--image-cache` allows to use a [local image cache](../configure-your-talos-cluster/images-container-runtime/image-cache.mdx)
...
Now the `_out/aws-amd64.raw.xz` contains the customized Talos AWS disk image which can be uploaded as an AMI to the [AWS](./cloud-platforms/aws.mdx).
