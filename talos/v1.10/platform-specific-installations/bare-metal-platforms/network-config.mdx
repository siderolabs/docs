...
If some advanced network configuration is required, it can be done via the [machine configuration](../../reference/configuration/overview.mdx) file.
...
See [kernel parameters reference](../../reference/kernel.mdx) for more details.
...
For the `metal` platform, the [interactive dashboard](../../deploy-and-manage-workloads/interactive-dashboard.mdx) can be used to edit the platform network configuration, also the configuration can be
created [manually](../../networking/metal-network-configuration.mdx).
...
The platform network configuration gets merged with other sources of network configuration, the details can be found in the [network resources guide](../../learn-more/networking-resources.mdx#configuration-merging").

## `nocloud` Network Configuration

Some bare-metal providers provide a way to configure network via the `nocloud` data source.
Talos Linux can automatically pick up this [configuration](../cloud-platforms/nocloud.mdx) when `nocloud` image is used.
