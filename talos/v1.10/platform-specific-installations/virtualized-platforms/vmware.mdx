...
<CodeBlock lang="sh">
  {`
curl -fsSLO https://raw.githubusercontent.com/siderolabs/talos/master/website/content/${version}/talos-guides/install/virtualized-platforms/vmware/cp.patch.yaml
  `}
</CodeBlock>
...
<CodeBlock lang="sh">
  {`
curl -fsSL "https://raw.githubusercontent.com/siderolabs/talos/master/website/content/${version}/talos-guides/install/virtualized-platforms/vmware/vmware.sh" | sed s/latest/${release}/ > vmware.sh
  `}
</CodeBlock>
...
The script downloads VMWare OVA with `talos-vmtoolsd` from <a href={`https://factory.talos.dev/?arch=amd64&cmdline-set=true&extensions=-&extensions=siderolabs%2Fvmtoolsd-guest-agent&platform=vmware&target=cloud&version=${release}`}>
  Image Factory
...
The following can be used to adjust the [EPHEMERAL](../../learn-more/architecture.mdx#file-system-partitions) disk size.
