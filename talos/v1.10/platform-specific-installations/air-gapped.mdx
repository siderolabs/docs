...
We will use the [QEMU](./local-platforms/qemu.mdx) provisioner available in `talosctl` to create a local cluster, but the same approach could be used to deploy Talos in bigger air-gapped networks.
...
- Requirements for the Talos [QEMU](./local-platforms/qemu.mdx) cluster
...
Please see [pull-through cache guide](../configure-your-talos-cluster/images-container-runtime/pull-through-cache.mdx) for an example using Harbor container registry with Talos.
