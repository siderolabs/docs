...
> Make sure your subnet does not overlap with `10.244.0.0/16` or `10.96.0.0/12` the [default pod and services subnets in Kubernetes](../../troubleshooting/troubleshooting.mdx#conflict-on-kubernetes-and-host-subnets).
...
We will create a [machine config patch](../../configure-your-talos-cluster/system-configuration/patching.mdx#rfc6902-json-patches) to use the AWS time servers.
You can create [additional patches](../../reference/configuration/v1alpha1/config.mdx) to customize the configuration as needed.
