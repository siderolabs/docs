...
Kernel parameters prefixed with `talos.config.auth.` are used to configure [OAuth2 authentication for the machine configuration](../security/machine-config-oauth.mdx).
...
One such example is to provide [a custom CA certificate](../security/certificate-authorities.mdx") via `TrustedRootsConfig` in the machine configuration:
