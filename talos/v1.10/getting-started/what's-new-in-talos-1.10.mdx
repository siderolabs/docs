...
For critical changes, refer to the [upgrade notes](../configure-your-talos-cluster/lifecycle-management/upgrading-talos.mdx).

## Breaking Changes

### UEFI Boot

Talos 1.10 now uses the `systemd-boot` [bootloader](../platform-specific-installations/bare-metal-platforms/bootloader.mdx) and [Unified Kernel Images (UKIs)](https://uapi-group.org/specifications/specs/unified_kernel_image/) for UEFI systems.
...
Use [Boot Assets](../platform-specific-installations/boot-assets.mdx) instead.
...
Talos introduces [user disk volumes](../configure-your-talos-cluster/storage-and-disk-management/disk-management.mdx#user-volumes) via the [`UserVolumeConfig`](../reference/configuration/block/uservolumeconfig.mdx) machine config.
The `.machine.disks` field is deprecated but remains for backward compatibility.

### Driver Rebind

A new machine config, [`PCIDriverRebindConfig`](../reference/configuration/hardware/pcidriverrebindconfig.mdx), allows rebinding PCI device drivers to different targets.

### Ethernet Configuration

Talos now supports `ethtool`-style [Ethernet configuration](../networking/ethernet-config.mdx) via [`EthernetConfig`](../reference/configuration/network/ethernetconfig.mdx).
...
[Imager](../platform-specific-installations/boot-assets.mdx) supports bootloader selection when generating disk images:
...
See [SELinux](../security/selinux.mdx) for details.
