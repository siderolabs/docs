...
- use initial boot [kernel command line parameters](../reference/kernel.mdx) to configure networking
...
Talos [dashboard](../deploy-and-manage-workloads/interactive-dashboard.mdx) provides a way to configure `META`-based network configuration for a machine using the console, but
...
The format of each section matches the respective network [`*Spec` resource](../learn-more/networking-resources.mdx) `.spec` part, e.g the `addresses:`
...
Following the [boot assets](../platform-specific-installations/boot-assets.mdx) guide, create a disk image passing the network configuration as a `--meta` flag:
