...
Ingress rules are configured as extra documents [NetworkDefaultActionConfig](../reference/configuration/network/networkdefaultactionconfig.mdx) and
[NetworkRuleConfig](../reference/configuration/network/networkruleconfig.mdx) in the Talos machine configuration:
...
> It is recommended that the configuration be [applied](../configure-your-talos-cluster/system-configuration/editing-machine-configuration.mdx) in `--mode=try` to ensure it is reverted in case of a mistake.
