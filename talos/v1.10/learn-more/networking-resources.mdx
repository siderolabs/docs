...
Talos network configuration subsystem is powered by [COSI](./controllers-resources.mdx).
...
As API access requires network connection, [`--mode=try`](../configure-your-talos-cluster/system-configuration/editing-machine-configuration.mdx)
...
The kernel [command line](../reference/kernel.mdx) is parsed for the following options:
