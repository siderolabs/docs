...
In this guide we assume that Talos is configured with default features enabled, such as [Discovery Service](../configure-your-talos-cluster/system-configuration/discovery.mdx) and [KubePrism](../kubernetes-infrastructure-and-management/kubeprism.mdx).
...
Some information can be gathered from the [Interactive Dashboard](../deploy-and-manage-workloads/interactive-dashboard.mdx) which is available on the machine console.

When running in the cloud the networking should be configured automatically.
Whereas when running on bare-metal it may need more specific configuration, see [networking `metal` configuration guide](../platform-specific-installations/bare-metal-platforms/network-config.mdx).

## Talos API

The Talos API runs on [port 50000](../learn-more/talos-network-connectivity.mdx).
...
Make sure that the firewall is not blocking port 50000, and [communication](../learn-more/talos-network-connectivity.mdx) on ports 50000/50001 inside the cluster.

### Client Configuration Issues

Make sure to use correct `talosconfig` client configuration file matching your cluster.
See [getting started](../getting-started/getting-started.mdx) for more information.
...
The [VIP](../networking/vip.mdx) should never be used as Talos API endpoint.
...
If minimum [system requirements](../getting-started/system-requirements.mdx) are not met, this might manifest itself in various ways, such as random failures when starting services, or failures to pull images from the container registry.
...
In the default mode, `talosctl health` uses information from the [discovery](../configure-your-talos-cluster/system-configuration/discovery.mdx) to get the information about cluster members.
...
Talos Linux uses [Discovery Service](../configure-your-talos-cluster/system-configuration/discovery.mdx) to discover other nodes in the cluster.
...
Talos Linux removes itself from the discovery service when it is [reset](../configure-your-talos-cluster/lifecycle-management/resetting-a-machine.mdx).
...
Make sure that a single member was [bootstrapped](../getting-started/getting-started.mdx#kubernetes-bootstrap).
...
See [etcd maintenance](../build-and-extend-talos/cluster-operations-and-maintenance/etcd-maintenance.mdx) guide.

### `etcd` Quorum is Lost

See [disaster recovery](../build-and-extend-talos/cluster-operations-and-maintenance/disaster-recovery.mdx) guide.
...
- make sure Kubernetes version is [supported](../getting-started/support-matrix.mdx) with this Talos release
...
Configure the correct internal IP address with [`.machine.kubelet.nodeIP`](../reference/configuration/v1alpha1/config.mdx#Config.machine.kubelet.nodeIP)
...
- DNS records
- Talos-builtin shared IP ([VIP](../networking/vip.mdx))
...
Talos runs a set of [controllers](../learn-more/controllers-resources.mdx) which operate on resources to build and support machine operations.
