...
Currently, only [STATE](../../learn-more/architecture.mdx#file-system-partitions) and [EPHEMERAL](../../learn-more/architecture.mdx#file-system-partitions) partitions can be encrypted.
...
- `tpm` - encrypt with the key derived from the TPM (strong, when used with [SecureBoot](../../platform-specific-installations/bare-metal-platforms/secureboot.mdx)).
