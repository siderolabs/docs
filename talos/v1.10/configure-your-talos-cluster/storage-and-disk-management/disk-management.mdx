...
To configure the `EPHEMERAL` (`/var`) volume, append the following [document](../../reference/configuration/block/volumeconfig.mdx) to the machine configuration:
...
> Note: Currently, encryption for `EPHEMERAL` and `STATE` volumes is configured using [another config document](../../reference/configuration/v1alpha1/config.mdx#Config.machine.systemDiskEncryption).

### `IMAGECACHE` Volume

This system volume is not provisioned by default, and it only gets created if the [Image Cache](../images-container-runtime/image-cache.mdx#configuration) feature is enabled.

See [Image Cache configuration](../images-container-runtime/image-cache.mdx#configuration") for more details.
...
To create a user volume, append the following [document](../../reference/configuration/block/uservolumeconfig.mdx) to the machine configuration:
