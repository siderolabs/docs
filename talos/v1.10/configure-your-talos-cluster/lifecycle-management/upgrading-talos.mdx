...
Each Talos version has a corresponding installer image, listed on the release page for the version, for example <a href={`https://github.com/siderolabs/talos/releases/tag/${release}`}>{release}</a>.
...
Kubernetes upgrades should be managed separately per [upgrading kubernetes](../../kubernetes-infrastructure-and-management/upgrading-kubernetes.mdx).
...
* new configuration documents:
  * [UserVolumeConfig](../../reference/configuration/block/uservolumeconfig.mdx)
  * [PCIDriverRebindConfig](../../reference/configuration/hardware/pcidriverrebindconfig.mdx)
  * [EthernetConfig](../../reference/configuration/network/ethernetconfig.mdx)
* deprecations:
  * `.machine.install.extensions` is ignored now, see [Boot Assets](../../platform-specific-installations/boot-assets.mdx) for alternative
  * `.machine.disks` is deprecated, use [User Volumes](../storage-and-disk-management/disk-management.mdx#user-volumes) instead
* change in behavior:
