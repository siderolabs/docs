...
import { release, nvidia_driver_release, nvidia_container_toolkit_release } from '/snippets/custom-variables.mdx';

NVIDIA GPUs that have nvlink support (for eg: A100) will need the [nvidia-fabricmanager](https://github.com/siderolabs/extensions/pkgs/container/nvidia-fabricmanager) system extension also enabled in addition to the [NVIDIA drivers](./nvidia-gpu.mdx).
...
Create the [boot assets](../../platform-specific-installations/boot-assets.mdx) or a custom installer and perform a machine upgrade which include the following system extensions:
