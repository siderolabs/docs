...
- Local cluster requirements for either [docker](../../platform-specific-installations/local-platforms/docker.mdx) or [QEMU](../../platform-specific-installations/local-platforms/qemu.mdx).
...
With a [QEMU](../../platform-specific-installations/local-platforms/qemu.mdx) local cluster, a bridge interface is created on the host.
...
With a [docker](../../platform-specific-installations/local-platforms/docker.mdx) local cluster we can use docker bridge IP, default value for that IP is `172.17.0.1`.
...
The caching registries can be configured via machine configuration [patch](../system-configuration/patching.mdx), equivalent to the command line flags above:
