...
The ISO image can built with the [imager](../../platform-specific-installations/boot-assets.mdx#imager) by passing an additional `--image-cache` flag:
...
The disk image can be built with the [imager](../../platform-specific-installations/boot-assets.mdx#imager) by passing an additional `--image-cache` flag:
...
See [disk management](../storage-and-disk-management/disk-management.mdx#machine-configuration) for more information on volume configuration.
