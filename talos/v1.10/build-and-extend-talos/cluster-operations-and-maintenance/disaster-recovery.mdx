...
If a control plane node is up but `etcd` isn't, wipe the node's [EPHEMERAL](../../learn-more/architecture.mdx#file-system-partitions) partition to remove the `etcd`
