...
The process of generating boot assets with overlays is described in the [boot assets guide](../../platform-specific-installations/boot-assets.mdx).

### Example: Booting a Raspberry Pi 4 with an Overlay

Follow the board specific guide for [Raspberry Pi](../../platform-specific-installations/single-board-computers/rpi_generic.mdx) to download or generate the metal disk image and write to an SD card.
...
> In this case, refer to [this guide on how to build a custom images](./building-images.mdx) using an imager.
