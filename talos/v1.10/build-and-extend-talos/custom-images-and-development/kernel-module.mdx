...
[System extensions](./system-extensions.mdx) in Talos provide ways to add files to the root filesystem and make it possible to run privileged containers as services.
...
System extensions without kernel modules work without this requirement, but there are a couple extra step s needed when adding kernel modules.
...
Follow the steps in [developing Talos](./developing-talos.mdx#prepare) to create a docker builder and run a local container registry before running this command.
...
We will build and use [imager](../../platform-specific-installations/boot-assets.mdx) to include our extension.
