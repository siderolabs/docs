...
* verifying the [image integrity](../../security/verifying-images.mdx)
...
<CodeBlock lang="sh">{`git checkout ${release}`}</CodeBlock>

## Set up the Build Environment

See [Developing Talos](./developing-talos.mdx) for details on setting up the buildkit builder.
...
When [customizing Linux kernel]( ./customizing-the-kernel.mdx), the source for the [`siderolabs/pkgs`](https://github.com/siderolabs/pkgs) repository can
be overridden with:
...
The [local registry](./developing-talos.mdx) running on `127.0.0.1:5005` can be used as well to avoid pushing/pulling over the network:
...
The [ISO image](../../platform-specific-installations/boot-assets.mdx) is built with the help of `imager` container image, by default `ghcr.io/siderolabs/imager` will be used with the matching tag:
