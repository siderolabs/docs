...
import { k8s_release, version } from '/snippets/custom-variables.mdx';
...
- This guide assumes <a href= {`../../talos/${version}/configure-your-talos-cluster/system-configuration/kubeprism`}>KubePrism</a> is enabled and configured on port `7445`
...
**Step 1.** Create a patch file that sets the CNI to `none`:
...
**Step 2.** Generate the machine config with the patch applied:
...
**Step 1.** Create a patch file that sets the CNI to `none` and disables kube-proxy:
...
<Note>
  Things to note:
    - This step applies only to the Helm and Cilium CLI methods, if you're using Omni, skip it. The CNI-disable patch for Omni clusters is applied via the cluster template in [Method 1](#method-1-omni-manifest-sync) instead.
    - After bootstrapping, Talos will hang at phase 18/19 with `retrying error: node not ready` — expected, since nodes aren't marked ready until a CNI is running. You have about 10 minutes to apply Cilium before the node retries.
</Note>
...
For Talos v1.14 and later, disable the CNI and kube-proxy:
...
<Note>On Talos versions 1.13 and earlier, use the deprecated `cluster.inlineManifests` field instead — see [inlineManifests and extraManifests](../advanced-guides/inlinemanifests) for the older format.</Note>
