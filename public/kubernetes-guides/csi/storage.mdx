...
Running a storage cluster can be a very good choice.
...
- **Piraeus / LINSTOR**: Replicated block storage built on LINSTOR and DRBD, with low overhead.
...
[Mayastor](https://github.com/openebs/Mayastor) is an OpenEBS project built in Rust utilizing the modern NVMEoF system. It has its own installation guide, but needs a couple of Talos-specific adjustments first.

#### Configure Mayastor for Talos
...
Follow these steps to install the Piraeus Operator and create a storage pool:

1. To install the Piraeus Operator V2, follow Piraeus's dedicated [how-to guide for Talos](https://piraeus.io/docs/stable/how-to/talos/), which covers the full installation process, including the necessary Talos-specific configuration.

2. Install the `linstor` plugin for `kubectl`, which you'll need to manage storage resources. Install it directly from the [kubectl-linstor repository](https://github.com/piraeusdatastore/kubectl-linstor), or via [krew](https://krew.sigs.k8s.io/):

   ```bash
   kubectl krew install linstor
   ```

3. With the plugin in place, create a device pool on a blank disk (one with no existing partition table) on your target node:
...
4. Define a StorageClass that uses the pool you just created. Run the following command to generate a `piraeus-sc.yml` file containing the StorageClass definition:
...
The NFS client is part of the [`kubelet` image](https://github.com/siderolabs/kubelet) maintained by the Talos team.
...
An S3-compatible object store is often built on top of one of the storage backends above, rather than run as a separate system.
For instance, you may have your block storage running with Mayastor but assign a Pod a large Persistent Volume to serve your object store.

Many storage systems include an S3-compatible object store.
These include some already covered on this page, such as [Rook/Ceph](https://rook.io/docs/rook/v1.20/Storage-Configuration/Object-Storage-RGW/object-storage/) and [SeaweedFS](https://github.com/seaweedfs/seaweedfs/wiki/Amazon-S3-API).
