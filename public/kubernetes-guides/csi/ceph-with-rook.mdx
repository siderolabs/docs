...
Default PodSecurity configuration prevents execution of privileged pods.
...
Depending on the size of the Ceph cluster and the availability of resources the Ceph cluster should become available, and with it the storage classes that can be used with Kubernetes Persistent Volumes.
...
By default, Rook configures Ceph to have 3 `mon` instances, in which case the data stored in `dataDirHostPath` can be regenerated from the other `mon` instances.
...
On Talos Linux, <a href={`../../talos/${version}/reference/cli#talosctl-wipe`}> disks can be wiped directly</a> using `talosctl`.

Run the following command for each disk on each node used in the Rook Ceph cluster.
