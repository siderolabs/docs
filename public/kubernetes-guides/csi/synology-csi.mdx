...
The focus of this guide is to allow a Kubernetes cluster running on Talos to provision Kubernetes storage (both dynamic or static) on a Synology NAS using a direct integration, rather than relying on an intermediary layer like Rook/Ceph or Mayastor.
...
Edit `config/client-info.yml` to configure the connection information for DSM.
...
```bash
kubectl apply -f deploy/kubernetes/v1.20/namespace.yml
kubectl label namespace synology-csi pod-security.kubernetes.io/enforce=privileged --overwrite
```

<Note> The node DaemonSet runs a privileged container so it can manage iSCSI devices on the host.
Without the privileged enforcement label, Pod Security Admission will emit warnings or block the pods.</Note>
