...
Before you begin, make sure you have the following. The prerequisites would be group into three areas:

- Tooling
- PowerStore access
- Talos node requirements
...
You'll need the following from your PowerStore administrator:
...
Use Talos Linux v1.13.3 or higher. Every node also needs four Talos system extensions:
...
Talos enables the [Pod Security Admission Policies](../security/pod-security#override-the-pod-security-admission-configuration) by default, which blocks the privileged containers the CSI driver requires.
