...
<Tabs>
  ...
  <Tab title="At bootstrap time">
    Reference the install manifest from the control plane machine configuration.

    <Note>
    This requires the node to have outbound HTTPS access to the manifest URL at bootstrap time. If your environment is air-gapped, see [inlineManifests and extraManifests](./inlinemanifests) for alternatives.
    </Note>
    ...
    **Step 4.** If this is a new cluster, bootstrap it. Run this **only once** per cluster:
