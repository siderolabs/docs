...
Labels outside this set—including the conventional Kubernetes role labels `node-role.kubernetes.io/<role>`—are rejected by the API server when applied by the node.
...
Talos can manage node taints through the `KubeNodeConfig` document. These taints are reconciled by Talos onto the Node object.
...
machine:
  nodeTaints:
    node.kubernetes.io/assignment: infra:NoSchedule
```
