...
import { k8s_prev_release, k8s_release } from '/snippets/custom-variables.mdx';

This guide covers upgrading Kubernetes on Talos Linux clusters.

For a list of Kubernetes versions compatible with each Talos release, see the [Support Matrix](../../talos/v1.10/getting-started/support-matrix).

For upgrading the Talos Linux operating system, see [Upgrading Talos](../../talos/v1.10/configure-your-talos-cluster/lifecycle-management/upgrading-talos)
...
See the [Reproducible Machine Configuration](../../talos/v1.10/configure-your-talos-cluster/system-configuration/reproducible-machine-configuration) guide for full instructions on handling machine configurations after version bumps.
...
The commands below reference `${k8s_release}` (the target Kubernetes version) and `${k8s_prev_release}` (the currently running version). Export these variables, replacing the placeholders with your values:

```bash
export k8s_release=<target-k8s-version>
export k8s_prev_release=<current-k8s-version>
```
...
Wait for the new pod definition to propagate, then check that the `kube-apiserver` pods are running on the control plane nodes:

```bash
kubectl get pods -n kube-system -l k8s-app=kube-apiserver -o wide
```
...
Wait for the new pod definition to propagate, then check that the `kube-controller-manager` pods are running on the control plane nodes:

```bash
kubectl get pods -n kube-system -l k8s-app=kube-controller-manager -o wide
```
...
Wait for the new pod definition to propagate, then check that the `kube-scheduler` pods are running on the control plane nodes:

```bash
kubectl get pods -n kube-system -l k8s-app=kube-scheduler -o wide
```
