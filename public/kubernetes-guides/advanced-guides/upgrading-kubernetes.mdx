...
Confirm that the `kube-proxy` `DaemonSet` image shown in the diff matches the target version (`v${k8s_release}`). If it shows the previous version, set `.cluster.proxy.image` in the machine configuration as described in the [Proxy](#proxy) step before applying.
