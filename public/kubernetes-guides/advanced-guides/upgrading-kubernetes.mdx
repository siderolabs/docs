...
import { k8s_prev_release, k8s_release, version } from '/snippets/custom-variables.mdx';

This guide covers upgrading Kubernetes on Talos Linux clusters.

For a list of Kubernetes versions compatible with each Talos release, see the <a href={`../../talos/${version}/getting-started/support-matrix`}>Support Matrix</a>.

For upgrading the Talos Linux operating system, see <a href={`../../talos/${version}/configure-your-talos-cluster/lifecycle-management/upgrading-talos`}>Upgrading Talos</a>.
...
The recommended method to upgrade Kubernetes on a Talos-native cluster is to use the `talosctl upgrade-k8s` command. If you're using Omni, see [Upgrade Kubernetes on Omni-managed clusters](#upgrade-kubernetes-on-omni-managed-clusters) below instead.
...
updating "kube-controller-manager" to version "${k8s_release}"
 > "172.20.0.2": starting update
 > update kube-controller-manager: ${k8s_prev_release} -> ${k8s_release}
 > skipped in dry-run
 > "172.20.0.3": starting update
...
A Talos version change might also cause changes in the bootstrap manifests.
The updated bootstrap manifests, inline manifests and extra manifests are applied to the cluster as part of the Kubernetes upgrade process.

If any services were deleted or disabled, the resources associated with them will be pruned.
For example if <a href={`../../talos/${version}/reference/configuration/v1alpha1/config#proxy`}>kube-proxy</a> was disabled in the machine configuration the resources associated with it would be deleted at this stage.
...
If you wish to manage a resource previously applied by Talos, you need to take the following steps:
...
3. Remove the `config.k8s.io/owning-inventory` annotation from the resources.
4. Re-apply the resource with your own field manager or remove the `metadata.managedFields` manually.
...
See the <a href={`../../talos/${version}/configure-your-talos-cluster/system-configuration/reproducible-machine-configuration`}> Reproducible Machine Configuration</a> guide for full instructions on handling machine configurations after version bumps.

## Upgrade Kubernetes on Omni-managed clusters

On a cluster managed by Omni, upgrade Kubernetes from the Omni UI rather than using `talosctl upgrade-k8s` directly:

1. Open the cluster in Omni.
2. Click the upgrade indicator next to the current Kubernetes version, or click **Update Kubernetes** on the cluster overview.
3. Select the target Kubernetes version and confirm.

Omni performs the same phases described above (pre-pulling images, updating control plane components, then kubelet on each node), non-disruptively to workloads.

<Note>
Unlike `talosctl upgrade-k8s`, Omni does not automatically re-apply Kubernetes bootstrap manifests as part of a Kubernetes upgrade, to avoid overwriting manifest changes you've applied manually. After upgrading, review the Bootstrap Manifests panel for the cluster in Omni and apply any changes shown there if they're appropriate.
</Note>

For a template-managed cluster, you can alternatively update the `kubernetes.version` field in the cluster template and sync it:
...
```bash
export k8s_release=<target-k8s-version>
```
...
```bash
talosctl -n <control-plane-ip> patch mc --mode=no-reboot -p "apiVersion: v1alpha1
...
<Tab title="Talos < v1.14">
```bash
talosctl -n <control-plane-ip> patch mc --mode=no-reboot -p "cluster:
  apiServer:
    image: registry.k8s.io/kube-apiserver:v${k8s_release}"
```
...
```bash
kubectl get pods -n kube-system -l k8s-app=kube-apiserver -o jsonpath='{.items[*].spec.containers[*].image}'
```
...
```bash
kubectl get pods -n kube-system -l k8s-app=kube-controller-manager -o jsonpath='{.items[*].spec.containers[*].image}'
```
...
Wait for the new pod definition to propagate, then confirm the running `kube-scheduler` pods are using the new image version:

```bash
kubectl get pods -n kube-system -l k8s-app=kube-scheduler -o jsonpath='{.items[*].spec.containers[*].image}'
```
...
Confirm that the `kube-proxy` `DaemonSet` image shown in the diff matches the target version (`v${k8s_release}`). If it shows the previous version, set the proxy image in the machine configuration as described in the [Proxy](#proxy) step before applying.
...
<Tab title="Talos v1.14+">
```bash
talosctl -n <node-ip> patch mc --mode=no-reboot -p "apiVersion: v1alpha1
kind: KubeletConfig
...
<Tab title="Talos < v1.14">
```bash
talosctl -n <node-ip> patch mc --mode=no-reboot -p "machine:
  kubelet:
    image: ghcr.io/siderolabs/kubelet:v${k8s_release}"
```
