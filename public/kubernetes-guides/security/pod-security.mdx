...
You can override the default PSA policy shown above by setting `cluster.apiServer.admissionControl` in the machine configuration. This works whether you're setting it before the cluster is created or patching an existing cluster.

Create and apply a config patch with the PSA configuration you want.

**For example**, to also warn on `baseline` violations instead of just `restricted`:
...
Here are some typical error messages you’ll run into when a pod violates a PSA policy, along with what went wrong and how to fix it.

1. **Pod denied because of restricted fields**:

```output wrap
Error from server (Forbidden): error when creating “pod.yaml”: pods “nginx” is forbidden: violates PodSecurity “baseline:latest”: privileged (container “nginx” must not set securityContext.privileged=true)
```

**What’s happening**: Your pod does not comply with the `baseline` profile. For example, the pod either allows privilege escalation or doesn’t force non-root user.
...
2. **Warnings for policy non-compliance**:

```bash wrap
Warning: would violate PodSecurity "restricted:latest": unrestricted capabilities (container "app" must set securityContext.capabilities.drop=["ALL"])
```

**What’s happening**: PSA allows the pod to run, but it still flags issues that break the policy, like missing capability drops in this example.

**Fix**: Explicitly drop capabilities, for example:
...
3. **Pod denied due to host-level access**:

```bash wrap
Error from server (Forbidden): pods "my-pod" is forbidden: violates PodSecurity "restricted:latest": hostPath volumes (volume "data") use of hostPath volumes is not allowed
```

**What’s happening**: Your pod uses a host-level resource (e.g., a hostPath volume) or other disallowed field under the restricted profile.

**Fix**: Remove or replace the hostPath (or other prohibited resource) and use a safer pattern (for example `emptyDir`, a CSI volume, etc.) in your pod spec.

4. **Invalid namespace label value**:

```bash wrap
Error from server (BadRequest): admission webhook "pod-security.admission.k8s.io" denied the request: invalid label value for pod-security.kubernetes.io/enforce
```

**What’s happening**: The label value you used on the namespace is not one of the allowed options (privileged, baseline, restricted).

**Fix**: Change the label to a valid value. For example:
