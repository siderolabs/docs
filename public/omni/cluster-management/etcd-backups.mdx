...
Omni uses the AWS S3 API for storing etcd backups. Providers that advertise S3 compatibility may work when configured with a custom `endpoint`, but Omni does not maintain an exhaustive compatibility list.

Compatibility with non-AWS providers is best effort and depends on how closely the provider matches AWS S3 behavior. Changes in client SDK behavior may affect third-party providers over time.
