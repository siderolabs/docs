...
### 7.3: Create storage directories

Omni uses SQLite alongside etcd for certain internal state. Both need to persist on the host so they survive container restarts and upgrades. These directories are mounted into the container in **Step 7.5**.
