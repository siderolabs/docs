...
<Note>If you are running an insecure registry make sure to set [insecure configuration
](https://github.com/siderolabs/image-factory/blob/main/docs/configuration.md#artifactsschematicinsecure). If you are running a registry with self-signed certificates make sure to mount the CA certificate to `/etc/ssl/certs` the container. If your registry is running on the same system you'll need to add `--net=host` to the command.</Note>
