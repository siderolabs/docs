...
<iframe width="560" height="315" src="https://www.youtube.com/embed/ExpIWRdrKEQ?si=EbRKeGkRplmZVaLv" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
To boot machines and connect them to Omni, you need to embed the self-signed CA certificate into Talos. Start by creating a `TrustedRootsConfig`:
...
Compress and base64 encode the configuration. Copy the output, you will need it when creating the installation media.

```bash
cat trustedrootsconfig.yaml | zstd --compress --ultra -21 | base64 -w 0
```

Copy the Omni kernel arguments using `omnictl`. You can also copy them from the Omni web interface with the **Copy Kernel Parameters** button on the home page.

<Note>If your shell still has `$OMNI_ENDPOINT` configured you may need to `unset OMNI_ENDPOINT` for `omnictl` commands to work.</Note>

```bash
omnictl jointoken kernel-args
```

<Note>If your `omni.internal` endpoint is not resolvable via DNS, update your machine config to set the SideroLink endpoint to the IP address of the Omni machine instead of a hostname.</Note>

Navigate to your airgapped image factory, work through each page, selecting the appropriate options for your setup, until you reach the **Customization** section.

In the **Extra kernel command line** arguments field, paste the following, replacing `<trusted-root-config>` with the encoded output and `<omni-kernel-args>` with the output from the omnictl command above:

  ```txt
  talos.config.early=<trusted-root-config> <omni-kernel-args>
  ```

  <Warning>
  Kernel arguments have a maximum length. If your certificate is too large (e.g. generated with a 4096-bit key), it will be silently truncated and fail to load without an error. Use a 2048-bit key for your CA if you're passing the certificate via kernel arguments.
  </Warning>

 Complete the remaining steps in the wizard and download your installation media.
