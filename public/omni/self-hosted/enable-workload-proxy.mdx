...
<Note>
**Using the Helm chart?** The chart handles workload proxy configuration, TLS, and ingress routing all in one place. See the [Workload Proxy section](../reference/helm-chart#workload-proxy-optional) of the Omni Helm Chart reference. You don't need the rest of this guide.
</Note>
...
For a full example including the cert-manager `Certificate` resource for the wildcard cert, see the [Traefik example](../reference/helm-chart#traefik-example) in the Omni Helm Chart reference.
