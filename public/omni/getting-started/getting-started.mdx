...
<Note>For Omni accounts that include a Talos Enterprise Linux entitlement (Talos 1.13 and later), your images and artifacts are served from Enterprise Image Factory.</Note>

<Tabs>
  <Tab title="CLI">

  To download a Talos Omni image, you must first create a preset. A preset is a saved configuration for generating installation media. The default preset produces a bare metal amd64 ISO. To customize the architecture, platform, or extensions, see the [CLI reference](/omni/reference/cli#omnictl-media-preset-create).

  <Note>`omnictl media` requires Omni v1.8.0 or later.</Note>

  ```bash
  omnictl media preset create my-preset
  ...
  ```bash
  omnictl media preset delete my-preset
  ```
  ...
  <Note>For Omni accounts with a Talos Enterprise Linux entitlement, the Schematic Ready page also shows a **Vulnerability Scan** section with the CVE count after VEX suppression, and a link to download the full report.</Note>
...
If you see the error `error: unknown command "oidc-login"`, ensure you’ve installed the kubectl-oidc-login plugin as described in the [prerequisites section](#prerequisites).

<Note>If your Omni account includes a Talos Enterprise Linux entitlement, you can access vulnerability reports for your cluster. See [Use Talos Enterprise Linux with Omni](../../talos-enterprise-linux/working-with-talos-enterprise-linux/use-talos-enterprise-linux-with-omni).</Note>
