...
Here is how to create a preset with machine user labels and download from it. This command creates a preset with the machine labels `environment=production` and `region=us-west` and downloads an image from that preset. For all available options, see the [CLI reference](/omni/reference/cli#omnictl-media-preset-create).

```bash
omnictl media preset create labels-preset --arch amd64 \
  --initial-labels environment=production,region=us-west
omnictl media download labels-preset
```

Presets persist in Omni after the image is downloaded. When you no longer need the preset, delete it:

```bash
omnictl media preset delete labels-preset
```

Behind the scenes, these commands generate a schematic and submit it to Image Factory. The schematic includes both the required kernel arguments for connecting to Omni and the labels you specified:
...
```bash
omnictl media download labels-preset --format pxe
```

This prints a PXE boot URL and exits without downloading a file.
</Info>
