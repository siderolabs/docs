...
Create an AWS preset and download the Talos AWS image. This command sets `--arch` to `amd64`, but you can set it to any supported architecture. For all available options, see the [CLI reference](/omni/reference/cli#omnictl-media-preset-create).

```bash
omnictl media preset create aws-preset --arch amd64 --platform aws
omnictl media download aws-preset
```

When you no longer need the preset, delete it:

```bash
omnictl media preset delete aws-preset
```
</Tab>
<Tab title="UI">
