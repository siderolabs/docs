...
* `67`: DHCP proxy broadcast port (for PXE boot DHCPDISCOVER)
* `4011`: DHCP proxy unicast port (for PXE boot DHCPREQUEST)
</Warning>
...
At the time of writing, it is `ghcr.io/siderolabs/omni-infra-provider-bare-metal:v0.8.1`, and this is what we shall use in this tutorial.
...
ghcr.io/siderolabs/omni-infra-provider-bare-metal:v0.8.1 \
...
```
7190a326decf   ghcr.io/siderolabs/omni-infra-provider-bare-metal:v0.8.1   "/provider --api-adv…"   About a minute ago   Up About a minute    omni-bare-metal-infra-provider
```
