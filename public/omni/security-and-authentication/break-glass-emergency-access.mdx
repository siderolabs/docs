...
<Warning>
The `os:operator` role does not permit `ApplyConfiguration`. Break glass is useful for inspection and operator-level recovery work, but it cannot push a corrected machine configuration to a machine that has already lost its connection to Omni, for example after a bad proxy or network configuration. Recovering such a machine means restoring SideroLink connectivity or going out of band, using the machine's BMC or Redfish interface to return it to maintenance mode, or [wiping it](../cluster-management/wipe-a-machine) and rejoining it.
</Warning>
