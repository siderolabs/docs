...
canonical: https://docs.siderolabs.com/talos/latest/configure-your-talos-cluster/storage-and-disk-management/disk-management
---
...
> Note: Currently, encryption for `EPHEMERAL` and `STATE` volumes is configured using [another config document](../../reference/configuration/v1alpha1/config#Config.machine.systemDiskEncryption).

### `IMAGECACHE` Volume

This system volume is not provisioned by default, and it only gets created if the [Image Cache](../images-container-runtime/image-cache#configuration) feature is enabled.
