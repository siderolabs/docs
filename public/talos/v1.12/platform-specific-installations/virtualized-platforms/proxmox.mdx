...
- Set **Type** to `host` (best performance, enables advanced instruction sets)
  - **Alternative for Proxmox < 8.0**: `kvm64` predates the x86-64-v2 microarchitecture level Talos [requires](../../getting-started/system-requirements#cpu-architecture-requirements), so add these feature flags to `/etc/pve/qemu-server/<vmid>.conf`:
    ```text
