...
import { release_v1_9, release_branch_v1_9} from '/snippets/custom-variables.mdx';
...
<CodeBlock lang="sh">
  {`
curl -fsSLO https://raw.githubusercontent.com/siderolabs/talos/master/website/content/${release_branch_v1_9}/talos-guides/install/virtualized-platforms/vmware/cp.patch.yaml
  `}
</CodeBlock>
...
<CodeBlock lang="sh">
  {`
curl -fsSL "https://raw.githubusercontent.com/siderolabs/talos/master/website/content/${release_branch_v1_9}/talos-guides/install/virtualized-platforms/vmware/vmware.sh" | sed s/latest/${release_v1_9}/ > vmware.sh
  `}
</CodeBlock>
...
The script downloads VMWare OVA with `talos-vmtoolsd` from <a href={`https://factory.talos.dev/?arch=amd64&cmdline-set=true&extensions=-&extensions=siderolabs%2Fvmtoolsd-guest-agent&platform=vmware&target=cloud&version=${release_v1_9}`}>
  Image Factory
