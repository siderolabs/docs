...
In this guide we assume that Talos is configured with default features enabled, such as [Discovery Service](../configure-your-talos-cluster/system-configuration/discovery) and [KubePrism](../configure-your-talos-cluster/system-configuration/kubeprism).
...
The [VIP](../networking/advanced/vip) should never be used as Talos API endpoint.
...
Configure the correct internal IP address with the `nodeIP` field of the [`KubeNodeConfig`](../reference/configuration/kubernetes/kubenodeconfig) document.
...
- DNS records
- Talos-builtin shared IP ([VIP](../networking/advanced/vip))
