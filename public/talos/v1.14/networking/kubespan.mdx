...
canonical: https://docs.siderolabs.com/talos/latest/networking/kubespan
---
...
KubeSpan and Cilium can generally be used together, but some Cilium configurations require specific settings to avoid asymmetric routing.

As of Talos 1.13, Cilium's native routing mode works with KubeSpan when `advertiseKubernetesNetworks: true` is set.
Configure Cilium with the following BPF settings:

```yaml
bpf:
  masquerade: true        # can be set to false if masquerading isn't needed
  hostLegacyRouting: true # required — eBPF host routing bypasses the kernel routing table and conflicts with KubeSpan
```

If you don't need native routing, Cilium's default configuration works with KubeSpan out of the box.
For advanced eBPF features without these constraints, use Cilium's built-in WireGuard encryption instead of KubeSpan.
