...
The old configuration is still supported for backwards compatibility.
...
Trimming is opt-in via the new [`FilesystemTrimConfig`](../reference/configuration/block/filesystemtrimconfig) document, which sets the global trim interval:
...
The default machine configuration for Talos 1.14+ includes a `FilesystemTrimConfig` document with a trim interval of one week,
so trimming is enabled by default for eligible filesystems.
For clusters which were upgraded from older versions the `FilesystemTrimConfig` document will be missing,
so trimming will be disabled until the document is added.
...
[`ExistingVolumeConfig`](../reference/configuration/block/existingvolumeconfig), [`ExternalVolumeConfig`](../reference/configuration/block/externalvolumeconfig)):
