...
A dedicated partition has its own mount, so the `mount.secure` option (`nosuid` and `nodev`, enabled by default) can be set per volume.
For dedicated `ETCD` and `LOG` volumes, `mount.secure` also enables `noexec`; dedicated `CRI` and `KUBELET` volumes remain executable.
Directory-backed volumes inherit the executable `EPHEMERAL` mount policy.
