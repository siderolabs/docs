...
The issuer and the signing key are configured with the [`KubeServiceAccountConfig`](../reference/configuration/kubernetes/kubeserviceaccountconfig) document: `issuer.issuerURL` becomes the API server's `--service-account-issuer`, and `issuer.privateKey` takes the key as PEM (not base64-encoded).
...
1.  Create the patch file, `machineconfig-patch.yaml`, fetching `BASE64_ENCODED_PRIVATE_KEY` from AWS SecretsManager as populated by the above module:
    ...
    issuer:
      issuerURL: https://${ISSUER_HOSTPATH}
      privateKey: |
    $(echo "${BASE64_ENCODED_PRIVATE_KEY}" | base64 -d | awk '{print "    " $0}')
    EOF
    ```
