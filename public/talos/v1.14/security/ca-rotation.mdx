...
> Verifying connectivity with existing PKI:
  - 172.20.0.2: OK (version ${release_v1_14})
  - 172.20.0.3: OK (version ${release_v1_14})
...
Each node `kubelet` will re-join the cluster with new client certificate.
...
Steps are similar [to the Talos API CA rotation](#manual-steps-for-talos-api-ca-rotation), but the Kubernetes API CA lives in the [`KubeAPIServerCAConfig`](../reference/configuration/kubernetes/kubeapiservercaconfig) document, so use:

- `acceptedCAs` of `KubeAPIServerCAConfig` in place of `.machine.acceptedCAs`;
- `issuingCA` of `KubeAPIServerCAConfig` in place of `.machine.ca` (on worker nodes only `acceptedCAs` is set);
- `kubeconfig` in place of `talosconfig`.

The Kubernetes API aggregator CA is configured separately with the [`KubeAggregatorCAConfig`](../reference/configuration/kubernetes/kubeaggregatorcaconfig) document, which has the same `issuingCA` and `acceptedCAs` fields.
