...
6. Replace the following information in the [`KubeNetworkConfig`](../reference/configuration/kubernetes/kubenetworkconfig) document of the generated `controlplane.yaml`:
    - `podSubnets[0]` with the value of the `networking.podSubnet` from the previous step
    - `serviceSubnets[0]` with the value of the `networking.serviceSubnet` from the previous step
    - `dnsDomain` with the value of the `networking.dnsDomain` from the previous step

   The existing cluster already runs a CNI, so remove the [`KubeFlannelCNIConfig`](../reference/configuration/kubernetes/kubeflannelcniconfig) document to keep Talos from deploying Flannel.

7. Go through the rest of `controlplane.yaml` and `worker.yaml` to customize them according to your needs, especially :
    - the [`KubeEtcdEncryptionConfig`](../reference/configuration/kubernetes/kubeetcdencryptionconfig) document should be either removed if you don't currently use `EncryptionConfig` on your `kube-apiserver` or set to the encryption configuration you use

8. Make sure that, on your current Kubeadm cluster, the first `--service-account-issuer=` parameter in `/etc/kubernetes/manifests/kube-apiserver.yaml` is equal to the `endpoint` value of the [`KubeClusterConfig`](../reference/configuration/kubernetes/kubeclusterconfig) document in `controlplane.yaml`.
