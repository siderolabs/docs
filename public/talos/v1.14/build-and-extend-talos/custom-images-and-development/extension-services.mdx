...
Extension services can be used to extend core features of Talos in a way that is not possible via [static pods](../../configure-your-talos-cluster/images-container-runtime/static-pods) or
...
<Warning>
    `ExtensionServiceConfig` is supported only when the extension service manifest declares `configuration: true`.
    Using it with a service that does not declare this dependency is unsupported and has undefined startup behavior.
