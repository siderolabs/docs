...
A Talos system extension is a container image with a specific folder structure.
System extensions can be built and managed using any tool that produces container images, e.g. `docker build`.

See [Building a System Extension](./building-a-system-extension) for the extension structure, the `rootfs` path restrictions, and how to build and install one.
