...
The contents of the `overrides` field are merged with the current base runtime specification, so only the fields that need to be adjusted should be included.

Applying, updating, or removing a [`CRIBaseRuntimeSpecConfig`](../../reference/configuration/cri/cribaseruntimespecconfig) regenerates the base runtime specification and restarts CRI automatically.
