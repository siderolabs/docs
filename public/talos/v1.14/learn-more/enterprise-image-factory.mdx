...
Once a build completes, its SBOM, VEX, and vulnerability scan report can all be downloaded from the same Image Factory UI page as the image's boot assets.
...
This differs from the public Image Factory's registry frontend, which performs an HTTP redirect to its backing CDN. When authentication is enabled, Enterprise Image Factory proxies all registry traffic through itself instead of redirecting, so the auth boundary applies consistently across every frontend.
