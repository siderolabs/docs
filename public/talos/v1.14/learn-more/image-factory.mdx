...
![Image factory home page](./images/image-factory-home.png)
...
The UI provides a way to list supported Talos Linux versions, a list of system extensions available for each release, and a way to generate schematic based on the selected system extensions.

The UI operations are equivalent to API operations.

## Find schematic ID from Talos installation

Image Factory always appends a "virtual" system extension with the version matching schematic ID used to generate the model.
...
Image Factory [can be deployed on-premises](../../../omni/self-hosted/run-image-factory-on-prem) to provide in-house asset generation.

Image Factory requires the following components:
