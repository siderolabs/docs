...
<CodeBlock lang="sh">
{`
talosctl images talos-bundle ${release_v1_14} >> images.txt
`}
</CodeBlock>
...
{`
mkdir -p _out/
docker run --rm -t -v $PWD/_out:/secureboot:ro -v $PWD/_out:/out -v $PWD/image-cache.oci:/image-cache.oci:ro ghcr.io/siderolabs/imager:${release_v1_14} iso --image-cache /image-cache.oci
`}
</CodeBlock>
...
{`
mkdir -p _out/
docker run --rm -t -v $PWD/_out:/secureboot:ro -v $PWD/_out:/out -v $PWD/image-cache.oci:/image-cache.oci:ro ghcr.io/siderolabs/imager:${release_v1_14} metal --image-cache /image-cache.oci
`}
</CodeBlock>
...
The image cache feature (for security reasons) should be explicitly enabled in the Talos configuration with the
[`ImageCacheConfig`](../../reference/configuration/cri/imagecacheconfig) document:
...
> Note: In Talos versions before v1.14, the image cache was enabled with the `.machine.features.imageCache.localEnabled` field of the `v1alpha1` config.
> That field is deprecated, but still honored, so existing configurations keep working after an upgrade.
> The two ways of enabling the image cache are mutually exclusive: if `.machine.features.imageCache` is set, the `ImageCacheConfig` document is rejected.
