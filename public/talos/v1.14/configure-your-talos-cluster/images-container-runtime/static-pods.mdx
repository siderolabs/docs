...
Each static pod is defined with a [`KubeStaticPodConfig`](../../reference/configuration/kubernetes/kubestaticpodconfig) document in the Talos machine configuration:
...
Use one document per static pod, with a unique `name` for each.
