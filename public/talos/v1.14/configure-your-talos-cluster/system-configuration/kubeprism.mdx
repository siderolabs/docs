...
canonical: https://docs.siderolabs.com/talos/v1.13/configure-your-talos-cluster/system-configuration/kubeprism
---
...
<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/VNRE64R5akM"
  title="Talos Linux KubePrism"
  frameborder="0"
  allow="accelerometer;
...
## Configuring KubePrism

KubePrism is enabled by default on port 7445, so most clusters won't need any additional configuration. If you do want to customize it,  use the [`KubePrismConfig`](../../reference/configuration/kubernetes/kubeprismconfig.mdx) document:
...
To disable KubePrism, remove the `KubePrismConfig` document.

<Note>The `port` specified should be available on every node in the cluster.</Note>
...
* for controlplane machines: `https://localhost:<api-server-local-port>` (`https://localhost:6443` in the default configuration)
* `https://<controlplane-address>:<api-server-port>` for every controlplane machine (based on the information from <a href={`/talos/${version}/configure-your-talos-cluster/system-configuration/discovery`}>
  Cluster Discovery
</a>)
