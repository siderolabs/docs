...
- If the field value is a list, the patch value is appended to the list, with the following exceptions:
  - values of the fields `KubeNetworkConfig.podSubnets` and `KubeNetworkConfig.serviceSubnets` are overwritten on merge
...
The goal is to update pod and service subnets and disable the default CNI (Flannel).
The subnet lists are replaced on merge, and the whole `KubeFlannelCNIConfig` document is removed with `$patch: delete`:
...
<iframe
  ...
  src="https://www.youtube.com/embed/0lrFpqi4bOI?si=KriagQyRXF5vQpqe"
  title="Multi-doc configurations with Talos Linux"
  frameBorder="0"
