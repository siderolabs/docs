...
- [`LVMVolumeGroupConfig`](../../reference/configuration/storage/lvmvolumegroupconfig) selects existing Talos volumes or whole disks and creates the physical volumes and volume group.
- [`LVMLogicalVolumeConfig`](../../reference/configuration/storage/lvmlogicalvolumeconfig) creates logical volumes inside the volume group.
...
An [`LVMVolumeGroupConfig`](../../reference/configuration/storage/lvmvolumegroupconfig) document requires:
...
An [`LVMLogicalVolumeConfig`](../../reference/configuration/storage/lvmlogicalvolumeconfig) document requires:
...
The following example creates two raw volumes with [`RawVolumeConfig`](../../reference/configuration/block/rawvolumeconfig) documents and selects their partition labels for `vg-pool`:
...
For details, see [User Volumes](./disk-management/user#user-volumes-on-lvm-logical-volumes) and the [`UserVolumeConfig` reference](../../reference/configuration/block/uservolumeconfig).
...
172.20.0.5   storage     LVMPhysicalVolumeStatus   /dev/sda1      1         /dev/sda1   vg-pool   50 GB     0 B       allocatable
172.20.0.5   storage     LVMPhysicalVolumeStatus   /dev/sdb1      1         /dev/sdb1   vg-pool   50 GB     20 GB     allocatable
```

```bash
$ talosctl get vgs
NODE         NAMESPACE   TYPE                   ID        VERSION   NAME      PERMISSIONS   SIZE     FREE    PVS   LVS
172.20.0.5   storage     LVMVolumeGroupStatus   vg-pool   1         vg-pool   writeable     100 GB   20 GB   2     1
```

```bash
$ talosctl get lvs
NODE         NAMESPACE   TYPE                    ID                VERSION   PATH                    VG        LAYOUT   SIZE    ACTIVE
172.20.0.5   storage     LVMLogicalVolumeStatus  vg-pool/lv-data   1         /dev/vg-pool/lv-data    vg-pool   linear   80 GB   active
```
...
First remove the [`LVMLogicalVolumeConfig`](../../reference/configuration/storage/lvmlogicalvolumeconfig) and [`LVMVolumeGroupConfig`](../../reference/configuration/storage/lvmvolumegroupconfig) documents from the machine configuration.
