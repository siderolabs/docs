...
* Talos exposes the directory through a bind mount, so it appears as a mount at `/var/mnt/<metadata.name>` while remaining on the `EPHEMERAL` filesystem.
* A `mount` block is not supported. The bind mount inherits mount attributes, including executable policy, from `EPHEMERAL`.
...
* It is not suitable for workloads that require predictable or enforceable storage quotas.
