...
This guide uses the default security group in OpenStack, with ports opened to allow connectivity from both inside and outside the group.
At a minimum, allow ports 6443 (Kubernetes API server) and 50000 (Talos API) from external sources, and it is recommended to allow communication over all ports from within the subnet.
...
First, download the <a href={`https://factory.talos.dev/image/376567988ad370138ad8b2698212367b8edcb69b5fd68c80be1f2ec7d603b4ba/${release_v1_14}/openstack-amd64.raw.xz`}> OpenStack image</a> from [Image Factory](https://factory.talos.dev/).
...
Create the private network that your cluster nodes will use to communicate:
...
Add a subnet to the private network and assign DNS resolvers:
...
Create a router that connects the network to the external `Ext-Net`:
...
Attach the subnet to the router so traffic can reach the gateway:
...
Create the three control plane instances on the private network:
