...
1. **Download the image disk** of the Talos version you wish to run:
  ...
  <CodeBlock lang="sh">
    {`
   wget "https://factory.talos.dev/image/18d0321d7fb289f707a76e1deeaa5c97e62209722cbf4bc533a5d51eb666885f/v{ release_v1_14 }/scaleway-amd64.raw.zst"
   `}
   </CodeBlock>
...
4. Name the snapshot *scaleway-amd64-v{ release_v1_14 }*, and use a Local Storage snapshot type.
...
The control plane taint is removed so that workloads can run on this single node, and the patch is applied to the control plane configuration only (the worker configuration has no taint to remove).

Generate the Talos config :
```bash
talosctl gen config talos-cluster https://$VM_IP:6443 --config-patch-control-plane=@controlplane.patch.yaml --output-dir _out/ --with-docs=false --with-examples=false
```
