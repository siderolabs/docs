...
docker run --rm -it
  ...
  ghcr.io/siderolabs/talos:${release_v1_14}

`}
</CodeBlock>

The machine configuration submitted to the container should have [host DNS](../../networking/host-dns) enabled with `forwardKubeDNSToHost` enabled, using the
`hostDNS` section of the [`ResolverConfig`](../../reference/configuration/network/resolverconfig) document.
