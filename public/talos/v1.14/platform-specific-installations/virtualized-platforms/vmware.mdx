...
Take note that we will also use a strategic merge patch when creating the configs so that the control plane nodes get some special information about the VIP we chose earlier.
...
<CodeBlock lang="sh">
  {`
curl -fsSLO https://raw.githubusercontent.com/siderolabs/docs/main/static/talos/${version_v1_14}/vmware/cp.patch.yaml
  `}
</CodeBlock>
...
Each command reports that the file is valid for cloud mode.
...
Download the `vmware.sh` script to your local machine and make it executable.
You can do this by issuing:

<CodeBlock lang="sh">
  {`
curl -fsSL "https://raw.githubusercontent.com/siderolabs/docs/main/static/talos/${version_v1_14}/vmware/vmware.sh" | sed s/latest/${release_v1_14}/ > vmware.sh
chmod +x vmware.sh
  `}
</CodeBlock>
...
The script downloads VMWare OVA with `talos-vmtoolsd` from <a href={`https://factory.talos.dev/?arch=amd64&cmdline-set=true&extensions=-&extensions=siderolabs%2Fvmtoolsd-guest-agent&platform=vmware&target=cloud&version=${release_v1_14}`}>
  Image Factory
