...
- **CPU tab:**
  ...
  `kvm64` predates the x86-64-v2 microarchitecture level Talos [requires](../../getting-started/system-requirements#cpu-architecture-requirements), so for Proxmox < 8.0 it needs the following feature flags added to `/etc/pve/qemu-server/<vmid>.conf`:
  ```text
