...
import { version_v1_6 } from '/snippets/custom-variables.mdx';
...
>
> `talosctl cluster create` derives Talos machine configuration version from the install image tag, so sometimes early in the development cycle (when new minor tag is not released yet), machine config version can be overridden with --talos-version={version_v1_6}.
