...
When using [predictable interface names](./predictable-interface-names), the interface name might not be `eth0`.
...
With the default configuration using [KubePrism and discovery](../configure-your-talos-cluster/system-configuration/kubeprism), pods can still reach the Kubernetes API through service discovery, so they remain unaffected by the VIP status.
