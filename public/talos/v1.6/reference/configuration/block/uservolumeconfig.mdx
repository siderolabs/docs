...
|`maxSize` |ByteSize |<details><summary>The maximum size of the volume, if not specified the volume can grow to the size of the</summary>disk.<br /><br />Size is specified in bytes, but can be expressed in human readable format, e.g. 100MB.</details> <details><summary>Show example(s)</summary>`maxSize: 50GiB`</details> | |
...
| Field | Type | Description | Value(s) |
|-------|------|-------------|----------|
| `match` | Expression | The Common Expression Language (CEL) expression to match the disk. <details><summary>Show example(s)</summary>`match: disk.size > 120u * GB && disk.size < 1u * TB` <br /> `match: disk.transport == "sata" && !disk.rotational && !system_disk`</details> | |
...
| `cipher` | string | Cipher to use for the encryption. Depends on the encryption provider. <details><summary>Show example(s)</summary>`cipher: aes-xts-plain64`</details> | `aes-xts-plain64`<br />`xchacha12,aes-adiantum-plain64`<br />`xchacha20,aes-adiantum-plain64`<br /> |
...
| `options` | []string | Additional --perf parameters for the LUKS2 encryption. <details><summary>Show example(s)</summary>```yaml{"\n"}options:{"\n"}    - no_read_workqueue{"\n"}    - no_write_workqueue{"\n"}```</details> | `no_read_workqueue`<br />`no_write_workqueue`<br />`same_cpu_crypt`<br /> |
