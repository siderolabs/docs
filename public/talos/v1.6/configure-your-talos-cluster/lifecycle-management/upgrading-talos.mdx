...
Each Talos version has a corresponding installer image, listed on the release page for the version, for example <a href={`https://github.com/siderolabs/talos/releases/tag/${release_v1_6 }`}>{release_v1_6 }</a>.
...
## Before Upgrade to {release_v1_6 }

There are no specific actions to be taken before an upgrade.

Please review the [release notes](../../getting-started/what's-new-in-talos) for any changes that may affect your cluster.
...
{`
  ...
  --image ghcr.io/siderolabs/installer:${release_v1_6 }
`}
</CodeBlock>
...
* [Ingress Firewall](../../networking/ingress-firewall) configuration: [NetworkRuleConfig](../../reference/configuration/network/networkruleconfig) and [NetworkDefaultActionConfig](../../reference/configuration/network/networkdefaultactionconfig).

Updates in [v1alpha1 Config](../../reference/configuration/v1alpha1/config):
