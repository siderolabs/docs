...
Each Talos version has a corresponding installer image, listed on the release page for the version, for example <a href={`https://github.com/siderolabs/talos/releases/tag/${release_v1_7 }`}>{release_v1_7 }</a>.
...
Unless explicitly told to `preserve` data, an upgrade will cause the node to wipe the [EPHEMERAL](../../learn-more/architecture#file-system-partitions) partition, remove itself from the etcd cluster (if it is a controlplane node), and make itself as pristine as is possible.
...
## Before Upgrade to {release_v1_7 }
...
{`
  ...
  --image ghcr.io/siderolabs/installer:${release_v1_7 }
`}
</CodeBlock>
...
* new configuration documents:
  ...
  * [PCIDriverRebindConfig](../../reference/configuration/hardware/pcidriverrebindconfig)
  * [EthernetConfig](../../reference/configuration/network/ethernetconfig)
* deprecations:
  * `.machine.install.extensions` is ignored now, see [Boot Assets](../../platform-specific-installations/boot-assets) for alternative
  * `.machine.disks` is deprecated, use [User Volumes](../storage-and-disk-management/disk-management#user-volumes) instead
* change in behavior:
