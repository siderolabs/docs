...
<CodeBlock lang="sh">
  {`
curl -fsSLO https://raw.githubusercontent.com/siderolabs/talos/master/website/content/${version_v1_8}/talos-guides/install/virtualized-platforms/vmware/cp.patch.yaml
  `}
</CodeBlock>
...
<CodeBlock lang="sh">
  {`
curl -fsSL "https://raw.githubusercontent.com/siderolabs/docs/main/static/talos/${version_v1_8}/vmware/cp.patch.yaml" | sed s/latest/${release_v1_8}/ > vmware.sh
  `}
</CodeBlock>
...
The script downloads VMWare OVA with `talos-vmtoolsd` from <a href={`https://factory.talos.dev/?arch=amd64&cmdline-set=true&extensions=-&extensions=siderolabs%2Fvmtoolsd-guest-agent&platform=vmware&target=cloud&version=${release_v1_8}`}>
  Image Factory
