...
> If this is your first use of Talos Linux, we recommend the [Quickstart](./quickstart) first, to quickly create a local virtual cluster in containers on your workstation.
>
> For a production cluster, extra steps are needed - see [Production Notes](./prodnotes).
...
For manual installation and other platforms please see the [talosctl installation guide](../platform-specific-installations).
...
If needed, see the documentation on [registry proxies](../configure-your-talos-cluster/images-container-runtime/pull-through-cache), local registries, and [airgapped installation](../platform-specific-installations/air-gapped).
...
For network booting and self-built media, see [Production Notes](./prodnotes#alternative-booting).
There are installation methods specific to specific platforms, such as pre-built AMIs for AWS - check the specific [Installation Guides](../platform-specific-installations).
...
> NOTE: For a production cluster, you should have three control plane nodes, and have the endpoint allocate traffic to all three - see [Production Notes](./prodnotes#control-plane-nodes).
...
> See [Production Notes](./prodnotes#configure-talos).
...
If you try to apply the machine config to a node, and get an error like the below, you need to specify a different installation disk:
...
> For information on customizing your machine configurations (such as to specify the version of Kubernetes), using [machine configuration patches](../configure-your-talos-cluster/system-configuration/patching), or customizing configurations for individual machines (such as setting static IP addresses), see the [Production Notes](./prodnotes#customizing-machine-configuration).
...
For production configurations, see [Production Notes](./prodnotes#decide-the-kubernetes-endpoint).
...
For a more in-depth discussion of Endpoints and Nodes, please see [talosctl](../learn-more/talosctl).
...
See [Production Notes](./prodnotes#default-configuration-file) for more information.
...
For a list of all the commands and operations that `talosctl` provides, see the [CLI reference](../reference/cli#talosctl).
