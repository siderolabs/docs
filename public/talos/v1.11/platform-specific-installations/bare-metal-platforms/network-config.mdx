...
canonical: https://docs.siderolabs.com/talos/latest/platform-specific-installations/bare-metal-platforms/network-config
---
...
If some advanced network configuration is required, it can be done via the [machine configuration](../../reference/configuration/overview) file.
...
created [manually](../../networking/metal-network-configuration).
...
<CodeBlock lang="sh">
{`
docker run --rm -i ghcr.io/siderolabs/imager:${release_v1_11} iso --arch amd64 --tar-to-stdout --meta 0x0a='{...}' | tar xz
docker run --rm -i --privileged ghcr.io/siderolabs/imager:${release_v1_11} image --platform metal --arch amd64 --tar-to-stdout --meta 0x0a='{...}' | tar xz
`}
</CodeBlock>
