---
title: "Xen Orchestra / XCP-ng"
description: "Creating Talos Kubernetes cluster using Xen Orchestra."
aliases: 
  - ../../../virtualized-platforms/xenorchestra-xcpng
---

In this guide, we will create a Kubernetes cluster using XCP-ng and Xen Orchestra.
...
For manual installation and other platforms, see the [talosctl installation guide](https://docs.siderolabs.com/talos/v1.11/getting-started/talosctl).

### Download the RAW disk image

To install Talos in Xen Orchestra, you will need the RAW image from [Image Factory](https://www.talos.dev/latest/talos-guides/install/boot-assets/#image-factory). To work with XCP-ng the image needs to be built for the Nocloud platform with the system extension `siderolabs/xen-guest-agent`. See this [example Talos image](https://factory.talos.dev/?arch=amd64&board=undefined&cmdline-set=true&extensions=-&extensions=siderolabs%2Fxen-guest-agent&platform=nocloud&target=cloud&version=1.11.5) you can use.
...
```bash
mkdir -p _out/
curl https://factory.talos.dev/image/53b20d86399013eadfd44ee49804c1fef069bfdee3b43f3f3f5a2f57c03338ac/${TALOS_VERSION}/nocloud-${ARCH}.raw.xz -L -o _out/nocloud-${ARCH}.raw.xz
xz -d _out/nocloud-${ARCH}.raw.xz
```
...
Before you begin, familiarize yourself with Talos' [system requirements](https://docs.siderolabs.com/talos/v1.11/getting-started/system-requirements) and allocate resources  
accordingly.
...
4. Set **Name** to `talos-v1.11.5-template` (for the control plane).
...
1. If the VM page isn't automatically open after creation, navigate to (**Sidebar Menu** >) **Home** > **VMs**, clear the filters, and type **talos-v1.11.5-template**.
2. Open the **Disk** tab.
...
3. In the **Info** section:
    - Select the template `talos-v1.11.5-template`.
...
provider "xenorchestra" {
  token = local.xoa_token
  # Must be ws or wss
  url = "ws://${local.xoa_url}"
}

data "xenorchestra_pool" "pool" {
  name_label = local.pool_name
}

data "xenorchestra_sr" "shared_storage" {
  name_label = local.sr_name
}

data "xenorchestra_network" "net" {
  name_label = local.network_name
  pool_id    = data.xenorchestra_pool.pool.id
}

resource "xenorchestra_vm" "cp" {
  memory_max           = 4 * local.size_1GB
  cpus                 = 2
  cloud_config         = file("${path.module}/controlplane.yaml")

  name_label           = "demo-talos-cp"
  name_description     = "Talos CP created with Terraform"
  template             = local.tpl_talos_id
...
For Secure Boot setup, see the **Secure Boot guide** in the [Talos documentation](https://docs.siderolabs.com/talos/v1.11/platform-specific-installations/bare-metal-platforms/secureboot) and **Enable Secure Boot for a Guest VM** in the [XCP-ng documentation](https://docs.xcp-ng.org/guides/guest-UEFI-Secure-Boot/#enable-secure-boot-for-a-guest-vm).

The easiest way to get started with Secure Boot is to download the [Secure Boot ISO](https://factory.talos.dev/image/53b20d86399013eadfd44ee49804c1fef069bfdee3b43f3f3f5a2f57c03338ac/v1.11.5/nocloud-amd64-secureboot.iso) and boot it on a UEFI-enabled VM with Secure Boot enabled in setup mode.
...
```text
factory.talos.dev/nocloud-installer-secureboot/53b20d86399013eadfd44ee49804c1fef069bfdee3b43f3f3f5a2f57c03338ac:v1.11.5
```
...
talosctl gen config talos-xcp-cluster https://${CONTROL_PLANE_ENDPOINT}:6443 \
  --install-image=factory.talos.dev/nocloud-installer-secureboot/53b20d86399013eadfd44ee49804c1fef069bfdee3b43f3f3f5a2f57c03338ac:v1.11.5 \
