...
For production-ready Talos deployments on Proxmox, we recommend the following "golden configuration":
...
- Set **Size** to 50GB+ for control planes, 100GB+ for workers (adjust based on CSI requirements)
...
- In Proxmox, go to your VM --> Options and ensure that `QEMU Guest Agent` is `Enabled` (only if using custom ISO with extension)
...
3. **Firewall rules**: If Proxmox VM firewall is enabled, allow DHCP traffic (UDP ports 67/68) and Talos management ports
...
- **Disk not found**: Check disk path using `talosctl get disks --insecure --nodes $CONTROL_PLANE_IP` and update `install.disk` in machine config if needed

### Secure Boot

For Secure Boot setup, see the [Secure Boot documentation](../../security/secure-boot).
