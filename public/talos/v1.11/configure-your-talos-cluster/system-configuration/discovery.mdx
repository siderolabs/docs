...
<Note>
Talos can operate with discovery disabled, and this does not affect day-to-day cluster operation.

When discovery is disabled, Talos no longer has access to peer and control plane endpoint information from discovery registries.
As a result, some behaviors change and certain features rely more heavily on Kubernetes API availability.

Behavioral changes when discovery is disabled:
...
- Endpoint resolution may depend on Kubernetes being available (for example via `kubectl get endpoints kubernetes`), which can require a functioning API server and load balancer.
- Worker nodes may be more dependent on control plane availability, as they cannot rely on discovery registries to obtain peer endpoint information during failures.

In air-gapped or otherwise restricted environments, outbound access to the public discovery service (`https://discovery.talos.dev`) may be unavailable.
In these cases, discovery can be disabled or replaced with a privately operated discovery service.

An enabled discovery service is required for [KubeSpan](../../networking/kubespan) to function.
</Note>
...
Node identity resource is preserved in the [STATE](../../learn-more/architecture#file-system-partitions) partition in `node-identity.yaml` file.
...
talos-default-controlplane-1   2         talos-default-controlplane-1   controlplane   Talos ${release_v1_11}   ["172.20.0.2","fd83:b1f7:fcb5:2802:8c13:71ff:feaf:7c94"]
talos-default-controlplane-2   1         talos-default-controlplane-2   controlplane   Talos ${release_v1_11}   ["172.20.0.3","fd83:b1f7:fcb5:2802:986b:7eff:fec5:889d"]
talos-default-controlplane-3   1         talos-default-controlplane-3   controlplane   Talos ${release_v1_11}   ["172.20.0.4","fd83:b1f7:fcb5:2802:248f:1fff:fe5c:c3f"]
talos-default-worker-1         1         talos-default-worker-1         worker         Talos ${release_v1_11}   ["172.20.0.5","fd83:b1f7:fcb5:2802:cc80:3dff:fece:d89d"]
talos-default-worker-2         1         talos-default-worker-2         worker         Talos ${release_v1_11}   ["172.20.0.6","fd83:b1f7:fcb5:2802:2805:fbff:fe80:5ed2"]
`}
</CodeBlock>
