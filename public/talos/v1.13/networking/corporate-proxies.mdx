...
## Connecting to Omni through a proxy

Machines managed by Omni have additional requirements behind a proxy: the SideroLink overlay prefix and `0.0.0.0` must be excluded from the proxy, and the SideroLink gRPC tunnel must be enabled where outbound UDP is blocked. See [Connect Machines to Omni Through a Corporate Proxy](../../../omni/omni-cluster-setup/connect-machines-to-omni-through-a-proxy).
