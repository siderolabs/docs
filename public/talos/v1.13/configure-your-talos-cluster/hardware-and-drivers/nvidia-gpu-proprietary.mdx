...
> To build a NVIDIA driver version not published by SideroLabs follow the instructions [here](./nvidia-gpu-proprietary)
...
you'll follow the [upstream instructions](https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/latest/getting-started.html) with only passing Helm chart values specific to Talos.

you'll disable the driver and toolkit components of the GPU operator since we have already enabled them as system extensions on Talos.
...
Talos does not allow direct shell access on the nodes, but you can still generate this report by follwing the steps below:
