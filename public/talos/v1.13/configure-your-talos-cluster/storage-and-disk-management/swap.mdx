...
canonical: https://docs.siderolabs.com/talos/latest/configure-your-talos-cluster/storage-and-disk-management/swap
---
...
Swap devices can be configured in the [Talos machine configuration](../../reference/configuration/block/swapvolumeconfig) similar to how [User Volumes](./disk-management/user) are configured.
...
Zswap is a compressed cache for swap pages that can help reduce the performance impact of swapping by keeping frequently accessed pages in memory.
Zswap can be enabled in the [Talos machine configuration](../../reference/configuration/block/zswapconfig):
...
Please note that zswap requires swap to be enabled on the system to function properly.
...
Current swap and zwap usage and limits can be checked using the `talosctl cgroups` [command](../../build-and-extend-talos/cluster-operations-and-maintenance/cgroups-analysis):
...
Swap can benefit some workloads by evicting inactive memory pages, keeping more RAM available for caches and buffers.
...
* `vm.swappiness`: controls how aggressively the kernel moves pages from RAM to swap.
  Higher values increase swapping. The default is `60`.
  Setting `130` makes the kernel more willing to use swap (assuming the swap device is fast, e.g., NVMe).
* `vm.page-cluster`: controls the swap read-ahead size (pages per I/O).
  The default value `3` reads up to `8` pages at once.
  Setting `0` disables read-ahead for swap pages, which can reduce unnecessary I/O on non-rotational devices.
