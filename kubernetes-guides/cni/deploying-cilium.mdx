...
In this guide we assume that [KubePrism](../advanced-guides/kubeprism.mdx) is enabled and configured to use the port 7445.
...
- After installing Cilium, `cilium connectivity test` might hang and/or fail with errors similar to
  ...
  This is expected, you can workaround it by adding the `pod-security.kubernetes.io/enforce=privileged` [label on the namespace level](../security/pod-security.mdx).
