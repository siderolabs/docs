...
The simplest way to use local storage is to use [user volumes](../../talos/v1.10/configure-your-talos-cluster/storage-and-disk-management/disk-management.mdx#user-volumes).
...
First, we will create a separate [user volume](../../talos/v1.10/configure-your-talos-cluster/storage-and-disk-management/disk-management.mdx#user-volumes) for the Local Path Provisioner to use.
Apply the following machine configuration patch:

> Note: make sure you have [enough space](../../talos/v1.10/configure-your-talos-cluster/storage-and-disk-management/disk-management.mdx#disk-layout) available to provision the user volume.
